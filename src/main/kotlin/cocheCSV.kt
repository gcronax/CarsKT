...
fun leerDatosInicialesCSV(ruta: Path): List<Cochecsv>
{
        ...
        coches = filas.mapNotNull { columnas ->
            if (columnas.size >= 5) {
                try {
                    ...
                    Cochecsv(id_planta,nombre_modelo, nombre_marca, consumo, hp)
                } catch (e: Exception) {
...
fun escribirDatosCSV(ruta: Path, coches: List<Cochecsv>){
    try {
